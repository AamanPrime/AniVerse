...
export default function SqlQueriesPage() {
  ...
  const predefinedQueries = [
    {
      id: 3,
      description: "Find users who have commented on episodes but never left a review",
      sql: `
        SELECT DISTINCT u.UserID, u.UserName
        FROM Users u
        JOIN Comments_ c ON u.UserID = c.UserID
    ...
    },
    {
      ...
      sql: `
        ...
        FROM Users u
        LEFT JOIN Comments_ c ON u.UserID = c.UserID
    ...
    {
      id: 5,
      description: "Find users with premium subscriptions who haven't watched any anime in the last 3 months",
      sql: `
    ...
    },
    {
      ...
      sql: `
        SELECT 
            ...
            STRING_AGG(Title, ', ') AS Animes
        FROM (
            SELECT 
                a.AnimeID,
                a.Title,
                jsonb_object_keys(a.Actors::jsonb) AS actor_name
            FROM Animes a
        ) AS actor_list
        GROUP BY actor_name
        HAVING COUNT(DISTINCT AnimeID) > 1
        ORDER BY AnimeCount DESC;
      `,
    },
    {
      id: 7,
      description: "Get all episodes of a specific anime with their watch count",
      sql: `
        SELECT e.AnimeID, a.Title, e.SeasonNumber, e.EpisodeNumber, COUNT(wh.UserID) AS ViewCount
        FROM Episodes e
        JOIN Animes a ON e.AnimeID = a.AnimeID
        LEFT JOIN WatchHistory wh ON e.AnimeID = wh.AnimeID
        WHERE e.AnimeID = 'ANM0000001'
        GROUP BY e.AnimeID, a.Title, e.SeasonNumber, e.EpisodeNumber
    ...
    {
      id: 10,
      description: "Get the distribution of ratings for each anime (histogram-like)",
      sql: `
    ...
    },
    {
      ...
      sql: `
        SELECT a.Title, e.SeasonNumber, e.EpisodeNumber, COUNT(c.CommentID) AS CommentCount
        FROM Episodes e
        JOIN Animes a ON e.AnimeID = a.AnimeID
        LEFT JOIN Comments_ c ON e.AnimeID = c.AnimeID 
                              AND e.SeasonNumber = c.SeasonNumber 
                              AND e.EpisodeNumber = c.EpisodeNumber
        GROUP BY a.Title, e.AnimeID, e.SeasonNumber, e.EpisodeNumber
  ...
  const executeQuerysql = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Failed to execute querysql");
      }

      const data = await response.json();
      setResults(data); // Set results to display in the UI.
    } catch (err) {
  ...
  const handlePredefinedQuerySelect = (query) => {
    setQuerysql(query.sql); // Set predefined SQL query into textarea.
  };

  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-black text-white p-6">
      <div className="max-w-6xl mx-auto">
        <h1 className="text-4xl font-extrabold text-red-500 mb-6 text-center">SQL Query Executor</h1>
  
        {/* Predefined Queries Section */}
        <div className="bg-gray-800 rounded-xl p-6 shadow-lg mb-8">
          <h2 className="text-xl font-bold text-red-500 mb-4">Predefined Queries</h2>
          <ul className="space-y-4">
            {predefinedQueries.map((query) => (
              <li key={query.id}>
                <button
                  onClick={() => handlePredefinedQuerySelect(query)}
        ...
        {/* Query Input Section */}
        <div className="bg-gray-800 rounded-xl p-6 shadow-lg mb-8">
          <label htmlFor="sql-querysql" className="block text-sm font-medium text-gray-300 mb-2">
            Enter your SQL query or select a predefined one above:
          </label>
          <textarea
            id="sql-querysql"
            value={querysql}
  ...
  <div className="bg-gray-800 rounded-xl p-6 shadow-lg">
    <h2 className="text-2xl font-bold text-red-500 mb-4">Query Results</h2>
    <div className="bg-gray-900 p-4 rounded-lg mb-4">
      <pre className="text-sm text-green-400 whitespace-pre-wrap">{querysql}</pre>
    </div>
          ...
          {results.map((row, index) => (
            <tr key={index} className="hover:bg-gray-700 transition-colors">
              {Object.values(row).map((value, idx) => (
                <td key={idx} className="px-4 py-2 text-sm text-gray-300">
                  {value !== null ? value.toString() : <span className="italic text-red-400">NULL</span>}
                </td>
              ))}
            </tr>
          ))}
  ...
  <div className="bg-gray-800 rounded-xl p-6 shadow-lg">
    <h2 className="text-2xl font-bold text-red-500 mb-4">Query Results</h2>
    <p className="text-gray-400 italic">No rows returned for this query.</p>
  </div>
) : null}
